...
<body>
<?php
        $gathering_details = " SELECT * FROM `pl_gatheringcremation` WHERE `id` = '$id'" ;
        $detail = mysqli_query($connection, $gathering_details);

        if (mysqli_num_rows($detail) > 0){ 
            while ($fetch_detail = mysqli_fetch_assoc($detail)) {
    ?>
<button type="button" onclick="window.location.href='includes/memorial_service.php'" class="submit-btn1">BACK</button>
