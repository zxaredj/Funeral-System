...
<body>
<?php
        $traditional_details = " SELECT * FROM `pl_traditionalcremation` WHERE `id` = '$id'" ;
        $detail = mysqli_query($connection, $traditional_details);

        if (mysqli_num_rows($detail) > 0){ 
            while ($fetch_detail = mysqli_fetch_assoc($detail)) {
    ?>
<button type="button" onclick="window.location.href='includes/traditional_service.php'" class="submit-btn1">BACK</button>
