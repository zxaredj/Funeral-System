...
<body>
<?php
        $direct_cremation_details = " SELECT * FROM `pl_directcremation` WHERE `id` = '$id'" ;
        $detail = mysqli_query($connection, $direct_cremation_details);

        if (mysqli_num_rows($detail) > 0){ 
            while ($fetch_detail = mysqli_fetch_assoc($detail)) {
    ?>
<button type="button" onclick="window.location.href='includes/direct_service.php'" class="submit-btn1">BACK</button>
    ...
    <h2 class="info-title">DECEASED PERSONAL INFORMATION</h2>
          ...
          <div class="inputs">
            ...
            <div class="inputs">
                    ...
                    <input value="<?php echo $fetch_detail['urn']; ?>" type="text" id="urn" name="urn">
            </div>

            <div class="inputs">
                    <label for="urn-price">Urn Price</label>
                    <input value="<?php echo $fetch_detail['urnPrice']; ?>" type="text" id="urn-price" name="urn-price">
            </div>
